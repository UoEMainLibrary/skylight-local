        ...
        foreach($solr[$uri_field] as $uri) {
            ...
            if ($pos === false)
            {
                ...
                if ($Lunapos !== false)
                {
                    ...
                    echo '<a href="'.$uri.'" title="Link to High resolution version of image" target="_blank">High resolution version of photo (opens in a new tab)</a>';
                }
                else{
...
<?php if(isset($solr[$bitstream_field]) && $link_bitstream) {
    ...
    foreach($solr[$bitstream_field] as $bitstream) {
        ...
        ?>
            <br>
            <iframe src="<?php echo base_url(); ?>theme/<?php echo $this->config->item('skylight_theme'); ?>/addons/PDF_Viewer/pdf_reader.php?url=<?php echo base_url() . substr($bitstreamUri, 2) ?>" title="PDF Viewer" width="700" height="900"></iframe>
            <br>
